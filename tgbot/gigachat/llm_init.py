...
from .prompts import BASIC_PROMPT, PROBLEM_PROMPT
import asyncio
...
async def get_response(user, user_input, problem: str | None = None):
    """Получение ответа от LLM для конкретного пользователя"""
    context = await get_user_context(user.id)  # Асинхронный вызов
